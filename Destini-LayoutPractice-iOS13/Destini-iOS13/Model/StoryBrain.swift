...
struct StoryBrain {
    ...
    mutating func getStoryText () -> String {
        if storyCounter <= storyTelling.count {
            return storyTelling[storyCounter].title
        } else {
            storyCounter = 0
            return storyTelling[storyCounter].title
        }
    ...
    mutating func getStoryOptions (button:Int) -> String {
        if button == 1 {
            return storyTelling[storyCounter].choice1
        } else if button == 2 {
            return storyTelling[storyCounter].choice2
        } else {
            return "error"
        }
    }
    
    mutating func buttonPressed(button:String) {
        if button == storyTelling[storyCounter].choice1 {
            storyCounter = storyTelling[storyCounter].choice1Destination
        } else if button == storyTelling[storyCounter].choice2 {
            storyCounter = storyTelling[storyCounter].choice2Destination
        } else {
            print ("error")
        }
